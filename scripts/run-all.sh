#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"

start_module() {
  local dir="$1"
  local name="$2"
  cd "$ROOT_DIR/$dir"
  if [[ -f server.pid ]] && ps -p "$(cat server.pid)" >/dev/null 2>&1; then
    echo "[skip] $name ya está corriendo (PID $(cat server.pid))"
    return 0
  fi
  echo "[start] $name"
  nohup ./mvnw -q spring-boot:run > server.log 2>&1 & echo $! > server.pid
  # espera breve y muestra línea de 'Tomcat started' si aparece
  for i in {1..10}; do
    sleep 1
    if rg -n "Tomcat started" server.log >/dev/null 2>&1; then
      rg -n "Tomcat started" server.log | tail -n1 || true
      break
    fi
  done
}

start_module legacy-api "legacy-api"
start_module bff-web "bff-web"
start_module bff-mobile "bff-mobile"
start_module bff-atm "bff-atm"

echo "Listo. PIDs:"
for m in legacy-api bff-web bff-mobile bff-atm; do
  if [[ -f "$ROOT_DIR/$m/server.pid" ]]; then
    printf "%-11s %s\n" "$m:" "$(cat "$ROOT_DIR/$m/server.pid")"
  fi
done

